from .utils.defaults import default_install_dir, default_symlink_dir
...
def search_files(dir, pattern='julia-'):
...
def get_julia_build(path):
    ver_cmd = [path, "--startup=no",
               "-e", "println(Base.GIT_VERSION_INFO.commit_short)"]
    try:
...
def list_julia(*, symlink_dir=None):
    ...
    print(f"Find {len(julias)} executables from {color.UNDERLINE}{symlink_dir}{color.END}:")  # nopep8
